...
#if DEBUG
    INFO("* Subscribed to topic: %s (fd = %d)*\n", orbData->o_name, fd);
#endif
}

boolean_T uORB_read_step(orb_metadata_t* orbData,
                         pollfd* eventStructObj,
                         void* busData,
...
//#if defined(MW_PX4_NUTTX_BUILD)
        int poll_ret = poll(eventStructObj, 1, blockingTimeout);
        static int error_counter = 0;
        if (poll_ret == 0) {
#if DEBUG
            ERR("Got no data within %.9lf second", blockingTimeout / 1000.0);
#endif
        } else if (poll_ret < 0) {
            if (error_counter < 10 || error_counter % 500 == 0) {
                /* use a counter to prevent flooding and slowing the system down */
#if DEBUG
                ERR("ERROR return value from poll(): %d", poll_ret);
#endif
            }
            error_counter++;
...
void uORB_read_terminate(const pollfd* eventStructObj) {
